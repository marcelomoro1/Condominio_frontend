      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-8">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm font-medium">Controle de prestação de serviços e parcerias</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">ID Fornecedor</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="ID do parceiro" value={form.ID_FORNECEDOR}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Descrição do Serviço</label>
                <div className="relative">
                  ...
                  <textarea 
                    ...
                    placeholder="Ex: Manutenção de Elevadores" value={form.DESCRICAO}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Início</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Término</label>
                  <div className="relative">
                    <Calendar size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                    <input 
                      type="date"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Valor Mensal / Total</label>
                <div className="relative">
              ...
              <button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-emerald-100 active:scale-[0.98] flex items-center justify-center gap-2">
                <PlusCircle size={18} />
                {editId ? "Atualizar Contrato" : "Registrar Contrato"}
              </button>
              
              {editId && (
                <button 
                  type="button"
                  onClick={() => { setEditId(null); setForm({ ID_FORNECEDOR: "", DESCRICAO: "", DATA_INICIO: "", DATA_FIM: "", VALOR: "" }); }}
        ...
        <div className="lg:col-span-8">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-lg font-bold text-slate-800">Contratos Vigentes</h2>
            <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{contratos.length} Ativos</span>
          </div>
            ...
            <table className="w-full text-left border-separate border-spacing-y-4">
              <thead>
                <tr className="text-slate-400">
                  <th className="px-6 py-2 text-xs font-bold uppercase tracking-widest">Contrato / Detalhes</th>
                  <th className="px-6 py-2 text-xs font-bold uppercase tracking-widest">Vigência</th>
              ...
              <tbody>
                {contratos.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-6 py-20 text-center text-slate-300 font-medium">Nenhum contrato registrado.</td>
                  </tr>
                ) : (
                  contratos.map((c: any) => (
                    <tr key={c.ID_CONTRATO} className="group">
                      ...
                      <td className="px-6 py-5 bg-slate-50/50 border-y border-slate-100 group-hover:bg-emerald-50/30 transition-colors">
                        <div className="flex flex-col text-xs font-medium text-slate-500 gap-1">
                          <span className="flex items-center gap-1.5"><Calendar size={12} className="text-slate-300" /> {new Date(c.DATA_INICIO).toLocaleDateString('pt-BR')}</span>
                          <span className="flex items-center gap-1.5"><Calendar size={12} className="text-slate-300" /> {new Date(c.DATA_FIM).toLocaleDateString('pt-BR')}</span>
                        </div>
                      </td>
