...
export default function PessoasPage() {
  ...
  return (
    <div className="min-h-screen bg-white p-8 lg:p-14">
      {/* Integrated Header Style from Dashboard */}
      <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-8">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm font-medium">Cadastre moradores, funcionários e parceiros</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Tipo de Cadastro</label>
                <div className="relative">
                  <UserCircle size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" />
                  <select 
                    className="w-full pl-12 pr-10 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-emerald-500/5 focus:bg-white focus:border-emerald-200 outline-none transition-all text-sm font-medium appearance-none"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Documento (CPF/CNPJ)</label>
                <div className="relative">
                  <CreditCard size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                  <input 
                    className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-emerald-500/5 focus:bg-white focus:border-emerald-200 outline-none transition-all text-sm font-medium"
                    placeholder="000.000.000-00" value={form.CPF_CNPJ}
              ...
              <button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-emerald-100 active:scale-[0.98]">
                {editId ? "Atualizar Cadastro" : "Finalizar Cadastro"}
              </button>
              
              {editId && (
                <button 
                  type="button"
                  onClick={() => { setEditId(null); setForm({ NOME: "", TIPO_PESSOA: "", CPF_CNPJ: "" }); }}
        ...
        <div className="lg:col-span-8">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-lg font-bold text-slate-800">Registros Ativos</h2>
            <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{pessoas.length} Total</span>
          </div>
            ...
            <table className="w-full text-left border-separate border-spacing-y-4">
              <thead>
                <tr className="text-slate-400">
                  ...
                  <th className="px-6 py-2 text-xs font-bold uppercase tracking-widest">Documento</th>
              ...
              <tbody>
                {pessoas.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-6 py-20 text-center text-slate-300 font-medium">Nenhum registro encontrado.</td>
                  </tr>
                ) : (
                  pessoas.map((p: any) => (
                    <tr key={p.ID_PESSOA} className="group">
                      <td className="px-6 py-5 bg-slate-50/50 rounded-l-[1.5rem] border-y border-l border-slate-100 group-hover:bg-emerald-50/30 transition-colors">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            <span className="font-bold text-slate-700 leading-tight">{p.NOME}</span>
                            <span className="text-xs text-slate-400 font-medium">{p.TIPO_PESSOA === 'FISICA' ? 'Pessoa Física' : 'Pessoa Jurídica'}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-5 bg-slate-50/50 rounded-r-[1.5rem] border-y border-r border-slate-100 text-right group-hover:bg-emerald-50/30 transition-colors">
                        <div className="flex justify-end gap-2">
                          ...
                          <button 
                            onClick={() => handleDelete(p.ID_PESSOA)} 
