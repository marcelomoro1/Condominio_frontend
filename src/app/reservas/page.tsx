    ...
    <div className="min-h-screen bg-white p-8 lg:p-14">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-16 gap-8">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Agenda de Reservas</h1>
            <p className="text-slate-400 text-sm font-medium">Controle de ocupação das áreas comuns</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    placeholder="ID cadastrado" value={form.ID_MORADOR}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Área Comum</label>
                <div className="relative">
                  <MapPin size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                  <input 
                    className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-emerald-500/5 focus:bg-white focus:border-emerald-200 outline-none transition-all text-sm font-medium"
                    placeholder="ID do espaço" value={form.ID_AREA_COMUM}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Data do Evento</label>
                <div className="relative">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Fim</label>
                  <div className="relative">
                    <Clock size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                    <input 
                      type="time"
              ...
              <button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-emerald-100 active:scale-[0.98] flex items-center justify-center gap-2">
                <PlusCircle size={18} />
                {editId ? "Atualizar Reserva" : "Confirmar Reserva"}
              </button>
              
              {editId && (
                <button 
                  type="button"
                  onClick={() => { setEditId(null); setForm({ ID_MORADOR: "", ID_AREA_COMUM: "", DATA_RESERVA: "", HR_INICIO: "", HR_FIM: "" }); }}
        ...
        <div className="lg:col-span-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{reservas.length} Agendamentos</span>
          </div>
            ...
            <table className="w-full text-left border-separate border-spacing-y-4">
              <thead>
                <tr className="text-slate-400">
                  <th className="px-6 py-2 text-xs font-bold uppercase tracking-widest">Reserva / Espaço</th>
                  <th className="px-6 py-2 text-xs font-bold uppercase tracking-widest">Horário Marcado</th>
              ...
              <tbody>
                {reservas.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-6 py-20 text-center text-slate-300 font-medium">Nenhuma reserva futura encontrada.</td>
                  </tr>
                ) : (
                  reservas.map((r: any) => (
                    <tr key={r.ID_RESERVA} className="group">
                      <td className="px-6 py-5 bg-slate-50/50 rounded-l-[1.5rem] border-y border-l border-slate-100 group-hover:bg-emerald-50/30 transition-colors">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            <span className="font-bold text-slate-700 leading-tight">Pessoa (ID: {r.ID_MORADOR})</span>
                            <span className="text-xs text-slate-400 font-medium">Área {r.ID_AREA_COMUM} • {new Date(r.DATA_RESERVA).toLocaleDateString('pt-BR')}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5 bg-slate-50/50 border-y border-slate-100 group-hover:bg-emerald-50/30 transition-colors">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold bg-white border border-slate-100 text-emerald-600 shadow-sm">
                          <Clock size={14} /> {r.HR_INICIO} - {r.HR_FIM}
                        </span>
                      </td>
